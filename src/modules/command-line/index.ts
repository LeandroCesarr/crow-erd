...
export async function handleConnectCommand(
  ...
  const [source, target] = await Promise.all(
    args.map(async (arg) => ({
      ...
      column: await recoil.snapshot.getPromise(
        nodeColumnSelector({
          nodeId: arg[0],
          columnId: arg[1],
        })
  ...
  const edgeToCreate = createEdge({
    ...
    sourceColumnId: createHandleId(source.node.id, source.column.id, 'source'),
    targetNodeId: target.node.id,
    targetColumnId: createHandleId(target.node.id, target.column.id, 'target'),
  });

  recoil.set(edgesAtom, (eds) => addEdge(edgeToCreate, eds));
}

export async function handleToggleElementsIdVisibility(
  recoil: CallbackInterface,
  command: TParsedCommand
): Promise<void> {
